...
local MinAva = 50 	--- How many ava until you leave the hunt?
local MaxAva = 400 	--- How many ava you begin the hunt with?
...
requestor = IpcRequestorSocket.New("Req socket", 30322)
...
function onWalkerSelectLabel(labelName)
	...
	if (labelName == "synchro") then
		Walker.Stop()
		while(not pass) do
			requestor:SendRequest("synchro")
			if (requestor:GetResponse() == "synchro") then
				pass = true
			end
		end
		Walker.Start()
	...
	elseif (labelName == "ref 11") then
		if (Self.ItemCount(Mpotion) <= MinMana) or (Self.ItemCount(Ava) <= MinAva) or ((LogoutStamina) and (Self.Stamina() < 840)) then
			gotoLabel("back 11")
		end
	...
	elseif (labelName == "checkstuff") then
		...
		Walker.ConditionalGoto((Self.ItemCount(237) < ESTMana) or (Self.ItemCount(7642) < MaxAva) or (Self.ItemCount(AmmoID) < ESTAmmo), "Resupply", "ToHunt")
	...
	elseif (labelName == "potions") then
		Walker.Stop()
		local mpuptobuy = math.floor((Self.Cap() - destcap) / 3.1) + Self.ItemCount(Mpotion)

		if (Self.ItemCount(Mpotion) < mpuptobuy) or (Self.ItemCount(Ava) < MaxAva) then
			Self.SayToNpc({"hi", "trade"}, 65)
			wait(2000)
			if (Self.ItemCount(Ava) < MaxAva) then
				BuyItems(Ava, MaxAva)
				wait(500)
			end
			wait(200, 500)
...
if (Self.ItemCount(Mpotion) >= 1) or (Self.ItemCount(Ava) >=1) then
                ...
                for spot = 0, MainBp:ItemCount() do
                    local item = MainBp:GetItemData(spot)               
                        if (item.id == Mpotion) or (item.id == Ava) then
...
----------------------- Functions ----------------------
Module.New("WG", function(Mod2)
	local spots = {
				{x = 33177, y = 31771, z = 6},
				{x = 33175, y = 31771, z = 6}
				}

	for i, spotlist in ipairs(spots) do 
		while Map.IsTileWalkable(spotlist.x, spotlist.y, spotlist.z) do
			Targeting.Stop()
       		Self.UseItemWithGround(3180, spotlist.x, spotlist.y, spotlist.z)
			wait(200)
		end
	end
	Targeting.Start()
